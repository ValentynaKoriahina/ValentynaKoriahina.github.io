...
export function processText(inputText: any): string  {
...
function convertBibleReferencesToLinks(text: string) {
  ...
  function replaceWithLink(match: any, book: any, chapter: any, verses: any, additionalReferences: any) {
    const originalReference = `${chapter}:${verses}${additionalReferences || ''}`;
    const cleanedReference = originalReference.replace(/(?:ср\.\с*)|;/g, '');

    return `<span class="bible-link" onclick="window.handleLinkClick('${book} ${cleanedReference}', '${book} ${originalReference}')">${book} ${originalReference}</span>`;
